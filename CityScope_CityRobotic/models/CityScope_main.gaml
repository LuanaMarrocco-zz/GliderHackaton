...
species building schedules: []{
	...
	aspect realistic {
		if(colorChange = true){
			draw shape color: rgb(102, 153, 255,100) depth:depth;
		}	
     	else{
...
experiment CityScopeMain type: gui virtual:true{	
	output {	
		display CityScopeVirtual  type:opengl background:#white draw_env:false virtual:true{
		...
		display CityScopeVirtualExperimental  type:opengl background:#white draw_env:false virtual:true{
			species table aspect:base refresh:false;	
			species road aspect: base refresh:false;
...
experiment CityScopeVolpeDemo type: gui parent:CityScopeMain{
	...
	output {		
		...
		display CityScopeTable   type:opengl background:#white fullscreen:1 toolbar:false rotate:180 synchronized:true
